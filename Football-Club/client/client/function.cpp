...
void serve_show()
{
	for(int n=0;n<receivedData.PLAYER_NUM;n++)
	{
		cout<<"俱乐部:"<<setw(10)<<left<<receivedData.playerTransfered[n].cname
			<<"球员:"<<setw(10)<<left<<receivedData.playerTransfered[n].name
			<<"年龄:"<<setw(10)<<left<<receivedData.playerTransfered[n].age
			<<"能力:"<<setw(10)<<left<<receivedData.playerTransfered[n].ability
			<<"年薪:"<<setw(10)<<left<<receivedData.playerTransfered[n].salary
			<<"转会费:"<<setw(10)<<left<<receivedData.playerTransfered[n].transfer_fee
			<<"进球总数:"<<setw(10)<<left<<receivedData.playerTransfered[n].total_goal
			<<"服役年限:"<<setw(10)<<left<<receivedData.playerTransfered[n].serve_year<<endl; 
	}
}
void rewrite_file()
{
	ofstream file("clubs.txt",ios::out);
	if(!file.is_open())
	{
		cout<<"写入文件失败"<<endl;
		exit(1); 
...
{
	cout<<"行政:"<<setw(10)<<left<<name<<"年龄:"<<setw(10)<<left<<age
